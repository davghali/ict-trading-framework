"""
PIPELINE BACKTEST v4 — ULTRA-FOCUS
===================================

INSIGHTS from v1→v3 analysis :
  - Over-filtering KZ on H1 killed good "none" trades → WR dropped
  - London Open + London KZ have TRUE edge (WR 43.6% / 36.7%, Exp +0.30R)
  - GBPUSD D1 + AUDUSD D1 = best D1 assets (WR 39-41%, PF 1.45-1.5)
  - XAGUSD + XAUUSD H1 = best H1 assets

v4 STRATEGY (narrow + deep) :

ASSETS (elite only) :
  - H1 : XAUUSD + XAGUSD (best metals)
  - D1 : GBPUSD + AUDUSD (best forex pairs)

KZ FILTER (H1 only — D1 doesn't care about KZ) :
  - H1 : london_kz + london_open ONLY (elite KZs with WR > 36%)
  - D1 : all KZ allowed
...
RISK MGMT :
  - Max 2 consec loss → cooldown 15 bars
  - Max 3 trades/day per asset

EXPECTED : narrow but high PF, genuine edge
"""
...
# H1 KZ whitelist (D1 gets all KZ)
KZ_WHITELIST_H1 = {"london_kz", "london_open"}

MAX_CONSEC_LOSS = 2
COOLDOWN_BARS = 15
MAX_TRADES_PER_DAY = 3

ASSETS_H1 = ["XAUUSD", "XAGUSD"]
ASSETS_D1 = ["GBPUSD", "AUDUSD"]
...
def backtest(symbol: str, tf: Timeframe) -> List[Trade]:
    ...
    for fvg in fvgs:
        ...
        for i in range(fvg_bar + 1, min(fvg_bar + ENTRY_WINDOW, len(df))):
            ...
            if use_kz_filter and kz not in KZ_WHITELIST_H1:
                continue
        ...
        trades.append(Trade(
            ...
            r=round(r_realized, 3), bars=exit_bar - entry_bar,
            kz=kz, fvg_size=round(float(fvg.size_in_atr), 2),
...
def run():
    print("=" * 80)
    print("ICT CYBORG PIPELINE BACKTEST v4 — ULTRA-FOCUS")
    print("=" * 80)
    print(f"H1 assets : {ASSETS_H1}  (KZ filter : {KZ_WHITELIST_H1})")
    print(f"D1 assets : {ASSETS_D1}  (all KZ allowed)")
    print(f"Min FVG size: {MIN_FVG_SIZE_ATR}  Min impulsion: {MIN_FVG_IMPULSION}")
    print(f"TP1/TP2 : {TP1_R}R / {TP2_R}R  Partial 50%  BE after TP1")
    print()
    ...
    for sym in ASSETS_H1:
        ...
        if ts:
            ...
            print(f"▶ {sym:7s} H1 : n={s['n']:4d} · WR {s['win_rate_pct']:5.1f}% · "
                  ...
                  f"{s['trades_per_month']:.1f}/mo · DD {s['max_dd_pct']:+.1f}% · "
                  f"Ret {s['final_return_pct']:+.1f}% · Ann {s['annualized_pct']:+.1f}%")
    ...
    for sym in ASSETS_D1:
        ...
        if ts:
            ...
            print(f"▶ {sym:7s} D1 : n={s['n']:4d} · WR {s['win_rate_pct']:5.1f}% · "
                  f"Exp {s['expectancy_R']:+.3f}R · PF {s['profit_factor']} · "
                  f"{s['trades_per_month']:.2f}/mo · DD {s['max_dd_pct']:+.1f}% · "
                  f"Ret {s['final_return_pct']:+.1f}% · Ann {s['annualized_pct']:+.2f}%")

    print("\n" + "=" * 80)
    print("GLOBAL v4 — ULTRA-FOCUS (elite assets + elite KZ only)")
    print("=" * 80)
    g = stats(all_trades, "GLOBAL v4")
    for k, v in g.items():
        if k == "by_kz":
            print(f"{k:18s} : {v}")
        else:
    ...
    out = {
        ...
        "config": {
            "assets_h1": ASSETS_H1,
            "assets_d1": ASSETS_D1,
            "kz_whitelist_h1": list(KZ_WHITELIST_H1),
    ...
    (REPORTS_DIR / f"pipeline_backtest_v4_{tsid}.json").write_text(json.dumps(out, indent=2, default=str))
    print(f"\nReport: pipeline_backtest_v4_{tsid}.json")
